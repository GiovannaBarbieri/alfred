...
export async function startAnnualReportUpdate(
  id: number,
  payload: AnnualReportUpdateRequest,
): Promise<AnnualReportUpdateResponse> {
  return request<AnnualReportUpdateResponse>(`/general-indicators/reports/${id}/updates`, {
    method: "POST",
...
export async function getAnnualReportUpdate(id: number): Promise<AnnualReportUpdateState> {
  return request<AnnualReportUpdateState>(`/general-indicators/reports/${id}/updates/current`);
}
